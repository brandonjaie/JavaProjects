 ...
 */
 ...
 /*
CRUD functionality Spring MVC web application without any JavaScript. Here are the key things to remember:
    1. The values used for RequestMappings, the names of your JSPs, and the names of your controller
        methods can be the same, but they do not have to be the same.
    2. If your controller method needs to get one or more values from the HTTP Request, you must have a
        parameter of type HttpServletRequest in your method signature.
    3. If your controller method needs to pass any data to its corresponding JSP, you must have a parameter
        of type Model in your method signature. Your controller method is then responsible for adding the data
        the must be passed to the JSP on the Model object.
    4. Every HTML form will have two endpoints: one that simply forwards control to the correct view so the
        form is displayed and one that processes the submitted form data.
    5. If we use Spring Form tags, we can have Spring automagically convert Java DTOs into form values and
        submitted form values into Java DTOs. 
 */
...
public class HomeControllerNoAjax {
    ...
    */
    ...
    public String addNewContactNoAjax(@Valid @ModelAttribute("contact") Contact contact, BindingResult result, HttpServletRequest req) {
        ...
        // #3 - Redisplay the add form if we find any validation errors
        if (result.hasErrors()) {
            return "newContactFormNoAjax";
        }
